...
| [`GetSchemaForGameRequest`](#getschemaforgamerequest) | `stats()->schema()` | `GameSchema` |
...
## GetSchemaForGameRequest

```text frame="none"
$connector->stats()->schema(int $appId, ?Language $language = null)

new GetSchemaForGameRequest($appId, $language)
```

<p><Badge text="ISteamUserStats" variant="note" size="small" />{' '}<Badge text="throws AppNotFoundException" variant="danger" size="small" /></p>

Every stat and achievement a game publishes, with the display names and descriptions a
player's own [achievements](#getplayerachievementsrequest) are read against. `language`
localises them; see the [`Language`](/php-steam-api-sdk/dto-reference#language) enum.

```php "language: Language::Polish"
use Fkrzski\SteamApiSdk\Enums\Language;

$schema = $connector->stats()->schema(appId: 381210, language: Language::Polish);

foreach ($schema->achievements as $achievement) {
    echo $achievement->apiName, ' — ', $achievement->name, PHP_EOL;
}
```

<Aside type="caution" title="An app publishing no schema is not a failure">
An app that exposes no stats and no achievements — a DLC, a tool, a runtime — answers
`200` with an empty game, which becomes a `GameSchema` with null names and empty lists.
Only an app ID Steam does not know raises the exception.
</Aside>
