...
Once you have a [configured connector](./configuration), three concepts carry the
...
request returns which DTO is listed in the [API reference](./api-reference).
...
SDK failure uniformly. The [API reference](./api-reference) notes which request throws
which exception.
