...
- [Guide](./guide) — the `SteamId` value object, sending requests, and the exception hierarchy.
- [Configuration](./configuration) — the connector, `SteamConfig` options, and rate limiting.
- [API reference](./api-reference) — every request, its parameters, return type, and errors.
- [Data objects](./dto-reference) — the DTOs and enums the SDK decodes responses into.
